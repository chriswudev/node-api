...
class Model {
  ...
  buildModel(attributes, options = {}, sortOrderField) {
      ...
      if (!attributes[sortOrderField]) {
        attributes = _.assign(attributes, {
          [sortOrderField]: {
            ...
            autoIncrement: true,
            field: _.snakeCase(sortOrderField),
          },
        });
      }
      sortOrderField &&
        this._fields.indexOf(sortOrderField) < 0 &&
        this._fields.push(sortOrderField);
    }
    ...
    this._sequelizeModel = sequelize.define(
      this._modelName,
      attributes,
      options
    );
  ...
  findOne(filter, transaction) {
    return this._sequelizeModel.findOne({
  ...
  save(object, transaction) {
    ...
    if (object.id) {
      ...
      resPromise = this._sequelizeModel.create(object, {
        transaction: transaction,
      });
    }
    return resPromise.then((object) =>
      dalUtils.prepareForClient(object, this._fields)
    );
  }
